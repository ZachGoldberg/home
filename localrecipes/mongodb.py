...
class Recipe(zc.recipe.egg.Eggs):
    ...
    def install_mongodb(self):
        ...
        if not downloads_dir:
            downloads_dir = self.buildout['buildout']['downloads-directory']
    ...
    def _build_start_script(self, installed):
        ...
        oplogSize = self.options.get('testoplogSize')
        if (os.getenv('SYSTEM_PROD') or os.getenv('SYSTEM_DEV') or
                os.getenv('SYSTEM_STAGING')):
            oplogSize = self.options.get('prodoplogSize')

        if 'testoplogSize' in self.options:
            del self.options['testoplogSize']
        if 'prodoplogSize' in self.options:
            del self.options['prodoplogSize']

        self.options['oplogSize'] = oplogSize
